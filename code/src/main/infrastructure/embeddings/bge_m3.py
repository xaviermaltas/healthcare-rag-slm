...
class BGEM3Embeddings:
    """BGE-M3 embeddings handler for medical documents using sentence-transformers"""
    ...
    async def initialize(self) -> bool:
        """Initialize the BGE-M3 model using sentence-transformers"""
        try:
    ...
    def _load_model(self) -> SentenceTransformer:
        """Load the BGE-M3 model using sentence-transformers (runs in thread)"""
    ...
    def _encode_batch(self, 
        ...
        """Encode a batch of texts using SentenceTransformer (runs in thread)"""
    ...
    async def encode(self, texts: List[str]) -> np.ndarray:
        """Encode texts and return dense embeddings array"""
        if not self.model:
            await self.initialize()
    ...
    async def health_check(self) -> bool:
        """Check if the embeddings model is healthy"""
        try:
            if not self.model:
                await self.initialize()
            
            # Try encoding a small test text
            test_embedding = await self.encode(["health check"])
